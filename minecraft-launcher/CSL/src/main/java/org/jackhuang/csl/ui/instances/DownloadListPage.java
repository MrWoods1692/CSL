...
import static org.jackhuang.csl.ui.FXUtils.stringConverter;
import static org.jackhuang.csl.util.i18n.I18n.i18n;
import static org.jackhuang.csl.util.javafx.ExtendedProperties.selectedItemPropertyFor;
...
public class DownloadListPage extends Control implements DecoratorPage, GameInstancePage.GameInstanceLoadable {
    ...
    private final ObjectProperty<CSLGameRepository.InstanceReference> instanceReference = new SimpleObjectProperty<>();
    ...
    public void loadInstance(CSLGameRepository repository, @Nullable GameInstanceID instanceId) {
        this.instanceReference.set(new CSLGameRepository.InstanceReference(repository, instanceId));
    ...
    private void search(String userGameVersion, RemoteAddonRepository.Category category, int pageOffset, String searchFilter, RemoteAddonRepository.SortType sort) {
        ...
        Task.supplyAsync(() -> {
            CSLGameRepository.InstanceReference instanceReference = this.instanceReference.get();
    ...
    protected CSLGameRepository.InstanceReference getInstanceReference() {
        if (instanceSelection) {
            @Nullable GameInstanceID instanceId = selectedInstance.get();
            return new CSLGameRepository.InstanceReference(instanceReference.get().repository(), instanceId);
        } else {
