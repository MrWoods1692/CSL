...
import org.jackhuang.csl.event.Event;
import org.jackhuang.csl.event.EventManager;
import org.jackhuang.csl.java.JavaRuntime;
import org.jackhuang.csl.modpack.ModAdviser;
...
import org.jackhuang.csl.setting.LauncherSettings;
import org.jackhuang.csl.setting.SettingsManager;
import org.jackhuang.csl.setting.DefaultIsolationType;
...
import org.jackhuang.csl.setting.LegacyGameSettingsMigrator;
import org.jackhuang.csl.setting.GameDirectory;
import org.jackhuang.csl.setting.ProxyType;
import org.jackhuang.csl.setting.SettingFileUtils;
import org.jackhuang.csl.setting.GameSettingsPresetID;
import org.jackhuang.csl.setting.GameInstanceIconType;
import org.jackhuang.csl.ui.FXUtils;
import org.jackhuang.csl.util.FileSaver;
...
/// CSL game repository implementation backed by a GameDirectory and per-instance game settings.
@NotNullByDefault
public final class CSLGameRepository extends DefaultGameRepository {
    ...
    @NotNullByDefault
    public record InstanceReference(CSLGameRepository repository, @Nullable GameInstanceID instanceId) {
    }

    /// Directory under the instance root that stores CSL-managed instance metadata.
    private static final String INSTANCE_METADATA_DIRECTORY = ".csl";
    ...
    public CSLGameRepository(GameDirectory gameDirectory) {
    ...
    public DefaultDependencyManager getDependency(DownloadProvider downloadProvider) {
        return new DefaultDependencyManager(this, downloadProvider, CSLCacheRepository.REPOSITORY);
    }
    ...
    /// Removes an instance from disk and clears its cached CSL settings state.
    @Override
    ...
    /// Returns the CSL-managed metadata directory under the instance root.
    ///
    /// This directory stores instance-scoped files owned by CSL.
    ...
    /// Returns the CSL-managed configuration directory under the instance metadata directory.
    ...
    /// Returns the CSL-managed state directory under the instance metadata directory.
    ...
    public static long getAutoAllocatedMemory(long available) {
        long usable = available - 512 * 1024 * 1024; // Reserve 512 MiB memory for off-heap memory and CSL itself
        if (usable <= 0) {
            return available;
        }
