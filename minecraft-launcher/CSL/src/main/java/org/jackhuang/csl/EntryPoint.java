...
import org.jackhuang.csl.util.SelfDependencyPatcher;
...
public final class EntryPoint {
    ...
    public static void main(String[] args) {
        ...
        System.getProperties().putIfAbsent("http.agent", "CSL/" + Metadata.VERSION);
    ...
    private static void createCSLDirectories() {
        if (!Files.isDirectory(Metadata.CSL_LOCAL_HOME)) {
            try {
                Files.createDirectories(Metadata.CSL_LOCAL_HOME);
                if (OperatingSystem.CURRENT_OS == OperatingSystem.WINDOWS) {
                    try {
                        Files.setAttribute(Metadata.CSL_LOCAL_HOME, "dos:hidden", true);
                    } catch (IOException e) {
                        LOG.warning("Failed to set hidden attribute of " + Metadata.CSL_LOCAL_HOME, e);
                    }
                }
            } catch (IOException e) {
                // Logger has not been started yet, so print directly to System.err
                System.err.println("Failed to create CSL directory: " + Metadata.CSL_LOCAL_HOME);
                e.printStackTrace(System.err);
                showErrorAndExit(i18n("fatal.create_csl_current_directory_failure", Metadata.CSL_LOCAL_HOME));
            }
        }

        if (!Files.isDirectory(Metadata.CSL_USER_HOME)) {
            try {
                Files.createDirectories(Metadata.CSL_USER_HOME);
            } catch (IOException e) {
                LOG.warning("Failed to create CSL user home " + Metadata.CSL_USER_HOME, e);
            }
        }
    }
    ...
    private static void checkWine() {
        if (OperatingSystem.isRunningUnderWine()) {
            LOG.warning("CSL is running under Wine or its distributions!");
