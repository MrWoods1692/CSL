 ...
 */
package org.jackhuang.csl.ui.export;
...
import org.jackhuang.csl.modpack.ModpackExportInfo;
...
public final class ExportWizardProvider implements WizardProvider {
    ...
    public ExportWizardProvider(CSLGameRepository repository, GameInstanceID instanceId) {
    ...
    private Task<?> exportWithLauncher(String modpackType, ModpackExportInfo exportInfo, Path modpackFile) {
        ...
        return new Task<>() {
            ...
            public void preExecute() throws Exception {
                Path dest;
                if (packWithLauncher) {
                    dest = tempModpack = Files.createTempFile("csl", ".zip");
                } else {
                    dest = modpackFile;
                }
            ...
            public void execute() throws Exception {
                ...
                try (Zipper zip = new Zipper(modpackFile)) {
                    ...
                    zip.putTextFile(exported.toJson(), ".csl/config/launcher-settings.json");
                    ...
                    for (String extension : FontManager.FONT_EXTENSIONS) {
                        ...
                        Path font = Metadata.CSL_LOCAL_HOME.resolve(fileName);
                        if (!Files.isRegularFile(font))
                            font = Metadata.CURRENT_DIRECTORY.resolve(fileName);
                        if (Files.isRegularFile(font))
                            zip.putFile(font, ".csl/" + fileName);
                    }
