...
import org.jackhuang.csl.game.CSLCacheRepository;
import org.jackhuang.csl.setting.*;
import org.jackhuang.csl.task.AsyncTaskExecutor;
...
import org.jackhuang.csl.util.platform.windows.Gdi32;
import org.jackhuang.csl.util.platform.windows.User32;
...
public final class Launcher extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        try {
            ...
            if (Metadata.CSL_LOCAL_HOME.toString().indexOf('=') >= 0) {
    ...
    private static void initializeSettingsRuntime() {
        ...
        CacheRepository.setInstance(CSLCacheRepository.REPOSITORY);
        CSLCacheRepository.REPOSITORY.directoryProperty().bind(Bindings.createStringBinding(() -> {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            LOG.info("CSL User Home: " + Metadata.CSL_USER_HOME);
            LOG.info("CSL Local Home: " + Metadata.CSL_LOCAL_HOME);
            LOG.info("CSL Jar Path: " + Lang.requireNonNullElse(JarUtils.thisJarPath(), "Not Found"));
            LOG.info("CSL Log File: " + Lang.requireNonNullElse(LOG.getLogFile(), "In Memory"));
    ...
    private static void setupJavaFXVMOptions() {
        if ("true".equalsIgnoreCase(System.getenv("CSL_FORCE_GPU"))) {
            LOG.info("CSL_FORCE_GPU: true");
        ...
        setUpAnimationFrameRate:
        {
            ...
            String animationFrameRate = System.getenv("CSL_ANIMATION_FRAME_RATE");
            if (animationFrameRate != null) {
                LOG.info("CSL_ANIMATION_FRAME_RATE: " + animationFrameRate);
        ...
        String uiScale = System.getProperty("csl.uiScale", System.getenv("CSL_UI_SCALE"));
        if (uiScale != null) {
            uiScale = uiScale.trim();

            LOG.info("CSL_UI_SCALE: " + uiScale);
    ...
    private static void checkConfigOwner() {
        ...
        if (Files.exists(Metadata.CSL_USER_HOME))
            files.add(Metadata.CSL_USER_HOME.toString());
