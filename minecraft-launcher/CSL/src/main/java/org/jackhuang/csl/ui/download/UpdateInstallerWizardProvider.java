...
import org.jackhuang.csl.download.game.GameAssetIndexDownloadTask;
import org.jackhuang.csl.download.game.LibraryDownloadException;
...
import org.jackhuang.csl.task.DownloadException;
...
public final class UpdateInstallerWizardProvider implements WizardProvider {
    ...
    public UpdateInstallerWizardProvider(@NotNull CSLGameRepository repository, @NotNull String gameVersion, @NotNull GameInstanceManifest manifest, @NotNull String libraryId, @Nullable String oldLibraryVersion) {
    ...
    public Object finish(SettingsMap settings) {
        ...
        for (Object value : settings.asStringMap().values()) {
            if (value instanceof RemoteVersion remoteVersion) {
                ...
                hints.add(new Task.StagesHint(String.format("csl.install.%s:%s", remoteVersion.getLibraryId(), remoteVersion.getSelfVersion())));
                if ("game".equals(remoteVersion.getLibraryId())) {
                    hints.add(new Task.StagesHint("csl.install.libraries"));
                    hints.add(new Task.StagesHint("csl.install.assets"));
                }
