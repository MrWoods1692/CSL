...
import org.jackhuang.csl.event.EventPriority;
...
import org.jackhuang.csl.ui.game.GameSettingsPage;
...
public class GameInstancePage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    private void checkSelectedInstance() {
        runInFX(() -> {
            ...
            CSLGameRepository repository = this.instanceReference.get().repository();
    ...
    public void setInstance(GameInstanceID instanceId, CSLGameRepository repository) {
        this.instanceReference.set(new CSLGameRepository.InstanceReference(repository, instanceId));
    }

    public void loadInstance(GameInstanceID instanceId, CSLGameRepository repository) {
    ...
    private void clearAssets() {
        ...
        CSLGameRepository.InstanceReference currentInstanceReference = instanceReference.get();
    ...
    public CSLGameRepository getRepository() {
        return Optional.ofNullable(instanceReference.get()).map(CSLGameRepository.InstanceReference::repository).orElse(null);
    }

    public @Nullable GameInstanceID getInstanceId() {
        return Optional.ofNullable(instanceReference.get())
                .map(CSLGameRepository.InstanceReference::instanceId)
                .orElse(null);
    }
    ...
    public interface GameInstanceLoadable {
        ...
        void loadInstance(CSLGameRepository repository, @Nullable GameInstanceID instanceId);
    }
}
