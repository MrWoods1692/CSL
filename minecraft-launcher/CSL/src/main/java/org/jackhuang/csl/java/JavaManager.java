 ...
 */
package org.jackhuang.csl.java;
...
import org.jackhuang.csl.game.GameJavaVersion;
import org.jackhuang.csl.game.JavaVersionConstraint;
...
import org.jackhuang.csl.util.CacheRepository;
...
import org.jackhuang.csl.util.platform.UnsupportedPlatformException;
...
public final class JavaManager {
    ...
    public static final CSLJavaRepository REPOSITORY = new CSLJavaRepository(Metadata.CSL_USER_HOME.resolve("java"));
    public static final CSLJavaRepository LOCAL_REPOSITORY = new CSLJavaRepository(Metadata.CSL_LOCAL_HOME.resolve("java"));
    ...
    private static Map<Path, JavaRuntime> searchPotentialJavaExecutables(boolean useCache) {
        Searcher searcher = new Searcher(Metadata.CSL_USER_HOME.resolve("javaCache.json"));
        if (useCache)
            searcher.loadCache();
        ...
        if (System.getenv("PATH") != null) {
            ...
            for (String path : paths) {
                // https://github.com/MrWoods1692/CSL/issues/4079
        ...
        if (System.getenv("CSL_JRES") != null) {
            String[] paths = System.getenv("CSL_JRES").split(File.pathSeparator);
            for (String path : paths) {
                try {
