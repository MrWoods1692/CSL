...
import org.jackhuang.csl.launch.DefaultLauncher;
...
import org.jackhuang.csl.util.platform.CommandBuilder;
 ...
 * @author huangyuhui
 */
public final class CSLGameLauncher extends DefaultLauncher {

    public CSLGameLauncher(GameRepository repository, GameInstanceManifest manifest, AuthInfo authInfo, LaunchOptions options) {
        this(repository, manifest, authInfo, options, null);
    }

    public CSLGameLauncher(GameRepository repository, GameInstanceManifest manifest, AuthInfo authInfo, LaunchOptions options, ProcessListener listener) {
        this(repository, manifest, authInfo, options, listener, true);
    }

    public CSLGameLauncher(GameRepository repository, GameInstanceManifest manifest, AuthInfo authInfo, LaunchOptions options, ProcessListener listener, boolean daemon) {
    ...
    private Path extractLwjglUnsafeAgent() throws IOException {
        String agentVersion = JarUtils.getAttribute("csl.lwjgl-unsafe-agent.version", null);
        if (agentVersion == null) {
            throw new IOException("Missing csl.lwjgl-unsafe-agent.version attribute");
        }
