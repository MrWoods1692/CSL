...
import org.jackhuang.csl.ui.SVGContainer;
...
public class AdvancedListBox extends ScrollPane {
    ...
    /**
     * Makes this list behave as a collapsible sidebar.
     * <p>
     * When {@code collapsed} is true, every {@link AdvancedListItem} switches
     * to the compact layout showing only its centered icon; when false, items
     * return to the usual layout with the icon on the left and the title on
     * the right. Category titles (if any) are hidden while collapsed.
     */
    public AdvancedListBox bindSidebarCollapsed(BooleanProperty collapsed) {
        for (Node child : container.getChildren()) {
            if (child instanceof ClassTitle) {
                child.visibleProperty().bind(collapsed.not());
                child.managedProperty().bind(collapsed.not());
            } else if (child instanceof AdvancedListItem) {
                AdvancedListItem item = (AdvancedListItem) child;
                item.setCompact(true);
                item.expandedProperty().bind(collapsed.not());
            } else if (child instanceof StackPane) {
                Node inner = ((StackPane) child).getChildren().isEmpty() ? null : ((StackPane) child).getChildren().get(0);
                if (inner instanceof AdvancedListItem) {
                    AdvancedListItem item = (AdvancedListItem) inner;
                    item.setCompact(true);
                    item.expandedProperty().bind(collapsed.not());
                }
            }
