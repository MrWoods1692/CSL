...
public final class SettingsPage extends ScrollPane {
    ...
    private CompletableFuture<Path> onExportLogs() {
        return CompletableFuture.supplyAsync(Lang.wrap(() -> {
            String nameBase = "csl-exported-logs-" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH-mm-ss"));
            ...
            if (recentLogFiles.isEmpty()) {
                ...
                try (var os = Files.newOutputStream(outputFile);
                    ...
                    zos.putNextEntry(new ZipEntry(getEntryName(entryNames, "csl-latest.log")));
                    LOG.exportLogs(zos);
                    zos.closeEntry();
                }
            }
