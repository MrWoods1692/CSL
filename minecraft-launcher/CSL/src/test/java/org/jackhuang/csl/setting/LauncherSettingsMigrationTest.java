...
public final class LauncherSettingsMigrationTest {
    ...
    /// Tests migrating the CSL 2.x selected offline account username into the current account ID.
    @Test
    public void migratesLegacyCSL2SelectedOfflineAccount() throws IOException {
        try (FileSystem fileSystem = Jimfs.newFileSystem(Configuration.unix())) {
            Path root = fileSystem.getPath("/launcher-settings-migration-tests");
            Files.createDirectories(root);
            Path config = Files.createTempFile(root, "csl2-selected-account-", ".json");
            Files.writeString(config, """
                    {
                      "_version": 0,
                      "auth": {
    ...
    public void preservesPatchSchemaAndUnknownFields() {
        LauncherSettings launcherSettings = Objects.requireNonNull(LauncherSettings.fromJson(JsonParser.parseString("""
                {
                  "$schema": "https://schemas.glavo.site/csl/launcher-settings/1.0.1",
                  "futureField": true
                }
                """).getAsJsonObject()));
        ...
        assertEquals("https://schemas.glavo.site/csl/launcher-settings/1.0.1",
