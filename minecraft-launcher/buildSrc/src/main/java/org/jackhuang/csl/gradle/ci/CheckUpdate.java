...
public abstract class CheckUpdate extends DefaultTask {
    ...
    public void run() throws Exception {
        ...
        try (PrintWriter writer = githubEnv.isBlank()
            ...
            addEnv.accept("CSL_COMMIT_SHA", buildMetadata.revision());
            addEnv.accept("CSL_VERSION", buildMetadata.version());
            addEnv.accept("CSL_TAG_NAME", "v" + buildMetadata.version());
            addEnv.accept("CSL_CI_DOWNLOAD_BASE_URI", buildMetadata.downloadBaseUri);
        }
    }
    ...
    private BuildMetadata fetchBuildInfo(Helper helper, String jobUri) throws IOException, InterruptedException {
        ...
        Pattern fileNamePattern = Pattern.compile("CSL-(?<version>\\d+(?:\\.\\d+)+)\\.jar");
