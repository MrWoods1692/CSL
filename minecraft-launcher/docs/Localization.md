...
CSL provides localization support for multiple languages.

This document describes CSL's support status for these languages and provides a guide for contributors who want to help with CSL localization.

## Supported languages

Currently, CSL supports the following languages:
...
CSL uses language tags that conform to the IETF BCP 47 standard.
...
CSL will derive a search list from the current language tag and search for resources in order.
...
CSL requires all pull requests that update documentation and localization resources to also update the resources
...
CSL welcomes anyone to participate in translation and contribution. 
...
We recommend that contributors submit a [feature request](https://github.com/MrWoods1692/CSL/issues/new?template=feature.yml) before providing a new language translation,
...
If you want to add support for a new language to CSL, please start by translating [`I18N.properties`](../CSL/src/main/resources/assets/lang/I18N.properties).
The vast majority of CSL's texts are in this file, and translating it will translate the entire interface.
...
After determining the language tag, please create a file named `I18N_<localization file suffix>.properties` (e.g., `I18N_en.properties`) next to the [`I18N.properties` file](../CSL/src/main/resources/assets/lang).
...
After translating part of the file, you can [build CSL yourself](./Contributing.md#build-csl), 
and your translations will be included in the compiled CSL.
If your computer's default environment is not the target language,
you can set the environment variable `CSL_LANGUAGE` to the language tag you just found from the table,
and CSL will automatically switch to that language.

At this point, you can push the file to GitHub and submit a PR to CSL.
The maintainers of CSL will complete the remaining steps for you.
...
In addition to localized files, CSL also supports localizing certain text fields in JSON. Localized text in JSON uses the following format:
...
For missing resources in a certain language, CSL supports a resource fallback mechanism,
...
1. Normalize Language Codes
   ...
   CSL will try to map it to a registered tag.

   For example, CSL will replace the language code `eng` with `en`.
...
2. Map Macrolanguages to Individual Languages
   ...
   and the macrolanguage usually refers to a specific individual language, CSL will replace it with that individual language.

   For example, `zh` (Chinese) usually actually refers to `cmn` (Mandarin), so we replace the language code `zh` with `cmn`.

3. Derive Script

   If the current language tag does not specify a script, CSL will try to derive the script according to the following rules in order:
...
After refining the language code, CSL will derive a list of language tags based on this language tag.

For example, for the language tag `en-US`, CSL will refine it to `en-Latn-US` and derive the following search list:
...
For the language tag `zh-CN`, CSL will refine it to `cmn-Hans-CN` and derive the following search list:
...
CSL will merge resources according to the priority of this list; for resources that are difficult to merge (such as font files), 
CSL will load the highest-priority resource found in this list.

If the current language uses a three-letter ISO 639 code, but there is also a corresponding two-letter code, 
CSL will map it to the two-letter code before searching for resources.

For example, if the current environment's language tag is `eng-US`, 
CSL will map it to `en-US` and then search for localization resources according to the above rules.

### Additional Rules for Chinese

CSL always adds `zh-CN` to the search list for all Chinese environments, 
...
To simplify documentation maintenance, CSL uses a macro mechanism to automatically maintain parts of the documentation content.
...
For more about macros, see [MacroProcessor.java](../buildSrc/src/main/java/org/jackhuang/csl/gradle/docs/MacroProcessor.java).
<!-- #END BLOCK -->
