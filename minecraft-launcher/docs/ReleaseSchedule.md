# CSL - Craft Something Launcher Release Schedule

<!-- #BEGIN LANGUAGE_SWITCHER -->
**English** | 中文 ([简体](ReleaseSchedule_zh.md), [繁體](ReleaseSchedule_zh_Hant.md))
<!-- #END LANGUAGE_SWITCHER -->

This document describes the CSL version release schedule starting from October 2025.

## Versioning Rules

### Version Branches

CSL has multiple **version branches**, each named in the format `<major>.<minor>` (e.g., `3.7`).

The major version is incremented only when there are significant architectural changes in CSL,
while the minor version is incremented regularly according to the release schedule.

### Version Types

CSL has two types of versions: **Stable** and **Beta**.

#### Stable Version

The CSL stable version is suitable for users who prioritize software stability.
...
The CSL beta version is suitable for users who want to experience new features first.
...
CSL has two main release channels: **Stable Channel** and **Beta Channel**.
They are used to release the CSL stable and beta versions, respectively.

To test CSL versions before official release, we will push updates to some users in advance.
Users can enable the "Preview CSL releases early" option on the "Settings > General" page to receive preview updates from the corresponding channel.
...
---
gantt
    title CSL Version Lifecycle (Example)
    section 3.11
