 ...
 */
package org.jackhuang.csl.util.gson;
...
/// Stores a raw JSON schema string and, when possible, its parsed CSL schema identifier.
///
/// The JSON representation is always a string. CSL-owned schemas use the following
/// fixed URL form, but other strings can still be represented as unparseable schemas:
///
/// `https://schemas.glavo.site/csl/<id>/<version>`
///
...
///
/// Parsed CSL schemas use the following compatibility policy:
///
/// - When the schema string is not parseable as an CSL schema URL, the file must be rejected.
...
/// @param parsed the parsed CSL schema identifier, or `null` when the string is not parseable
/// @author Glavo
@JsonSerializable
...
public record JsonSchema(String value, @Nullable Parsed parsed) {
    ...
    /// The CSL schema URL prefix.
    private static final String URL_PREFIX = "https://schemas.glavo.site/csl/";
    ...
    /// Creates a parsed CSL schema from an ID and version.
    ///
    ...
    /// Creates a schema from a parsed CSL schema identifier.
    ...
    /// Parses an CSL schema URL, returning `null` for any other string.
    ...
    /// Returns whether this schema string is parseable as an CSL schema URL.
    public boolean isParsed() {
        return parsed != null;
    }

    /// Returns the parsed CSL schema ID, or `null` when the schema string is not parseable.
    public @Nullable String id() {
        return parsed != null ? parsed.id : null;
    }

    /// Returns the parsed CSL schema version, or `null` when the schema string is not parseable.
    ...
    /// Parsed identifier for an CSL schema URL.
    ///
    /// @param id the stable schema identifier
    /// @param version the schema version
    @NotNullByDefault
