 ...
 */
package org.jackhuang.csl.modpack.modrinth;
...
public class ModrinthInstallTask extends Task<Void> {
    ...
    public ModrinthInstallTask(DefaultDependencyManager dependencyManager, Path zipFile, Modpack modpack, ModrinthManifest manifest, GameInstanceID instanceId, String iconUrl) {
        ...
        for (Map.Entry<String, String> modLoader : manifest.getDependencies().entrySet()) {
            switch (modLoader.getKey()) {
                ...
                case "forge":
                    ...
                    break;
                case "neoforge":
                // https://github.com/MrWoods1692/CSL/pull/5170
                case "neo-forge":
        ...
        dependents.add(new ModpackInstallTask<>(zipFile, run, modpack.getEncoding(), subDirectories, any -> true, config).withStage("csl.modpack"));
        dependents.add(new MinecraftInstanceTask<>(zipFile, modpack.getEncoding(), subDirectories, manifest, ModrinthModpackProvider.INSTANCE, manifest.getName(), manifest.getVersionId(), repository.getModpackConfiguration(instanceId)).withStage("csl.modpack"));
