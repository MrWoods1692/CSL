 ...
 */
package org.jackhuang.csl.download;

import org.jackhuang.csl.game.*;
import org.jackhuang.csl.task.Task;
import org.jackhuang.csl.util.SimpleMultimap;
...
import static org.jackhuang.csl.download.LibraryAnalyzer.LibraryType.*;
...
public class MaintainTask extends Task<GameInstanceManifest> {
    ...
    public static GameInstanceManifest maintain(GameRepository repository, GameInstanceManifest manifest) {
        ...
        if (!libraries.isEmpty()) {
            // CSL once use log4j-patch to prevent virus. But now, we only modify log4j2.xml.
    ...
    private static GameInstanceManifest maintainGameWithCpwModLauncher(GameRepository repository, GameInstanceManifest manifest) {
        ...
        if (libraryAnalyzer.has(OPTIFINE)) {
            Library cslTransformerDiscoveryService = new Library(new Artifact("org.jackhuang.csl", "transformer-discovery-service", "1.0"));
            Optional<Library> optiFine = manifest.getLibraries().stream().filter(library -> library.is("optifine", "OptiFine")).findAny();
            boolean libraryExisting = manifest.getLibraries().stream().anyMatch(library -> library.is("org.jackhuang.csl", "transformer-discovery-service"));
            optiFine.ifPresent(library -> {
                builder.addJvmArgument("-Dcsl.transformer.candidates=${library_directory}/" + library.getPath());
                if (!libraryExisting) builder.addLibrary(cslTransformerDiscoveryService);
                Path libraryPath = repository.getLibraryFile(manifest, cslTransformerDiscoveryService);
                try (InputStream input = MaintainTask.class.getResourceAsStream("/assets/game/CSLTransformerDiscoveryService-1.0.jar")) {
                    Files.createDirectories(libraryPath.getParent());
                    Files.copy(Objects.requireNonNull(input, "Bundled CSLTransformerDiscoveryService is missing."), libraryPath, StandardCopyOption.REPLACE_EXISTING);
                } catch (IOException | NullPointerException e) {
                    LOG.warning("Unable to unpack CSLTransformerDiscoveryService", e);
                }
            });
        }
