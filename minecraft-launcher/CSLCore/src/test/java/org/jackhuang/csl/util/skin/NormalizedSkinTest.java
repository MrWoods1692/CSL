 ...
 */
package org.jackhuang.csl.util.skin;
...
public class NormalizedSkinTest {
    private static NormalizedSkin getSkin(String name, boolean slim) throws InvalidSkinException {
        String path = Paths.get(String.format("../CSLCore/src/main/resources/assets/img/skin/%s/%s.png", slim ? "slim" : "wide", name)).normalize().toAbsolutePath().toUri().toString();
        return new NormalizedSkin(new Image(path));
    }

    @Test
    @EnabledIf("org.jackhuang.csl.JavaFXLauncher#isStarted")
